    ...
    while open_set:
        ...
        for action in ACTIONS:
            ...
            if (
                ...
                next_cell not in closed_set
            ):
                if parent[row][col] is None:  # Not visited
                    parent[row][col] = current
                    actions[row][col] = action
                    open_set.add(next_cell)
    
    cell = goal
    if parent[goal[0]][goal[1]] is not None:  # Goal is reachable
        while cell != start:
            action = actions[cell[0]][cell[1]]
    ...
    while open_set:
        ...
        for action in ACTIONS:
            ...
            if 0 <= row < n_rows and 0 <= col < n_cols:
                if neighbor not in obstacles and neighbor not in closed_set and neighbor not in open_set:
    ...
    while open_set:
        ...
        for action in ACTIONS:
            ...
            if (
                ...
                if neighbor not in cost_so_far or new_cost < cost_so_far[neighbor]:
                    cost_so_far[neighbor] = new_cost
                    priority = new_cost + heuristic(row, col)
                    parent[row][col] = current
                    actions[row][col] = action
